"""Real-PG stress test for the asyncpg segfault under concurrent cancellation.

Background (issues #7, #8):
    The DatabaseManager sync facade (fetchval_sync, execute_sync, ...) was
    segfaulting on production (`rag-research-eai` revisions -00078-bsc and
    -00079-j4m) when called concurrently with task cancellation. Crash
    ...
    code is never actually loaded. This file loads the real asyncpg + real
    PostgreSQL + real event loop and reproduces the production pattern.

What the test does:
    1. Spins up a real DatabaseManager against a real PostgreSQL
    2. Fires N concurrent fetchval_sync calls (default: 50)
    3. Cancels half of them mid-flight via asyncio.CancelledError
       (simulated by submitting tasks to a fresh asyncio loop that we
       then cancel — but actually we use the production pattern: the
...
Skipping:
    The test skips when POSTGRES_HOST is not set, so lightweight CI (no
    real PG) doesn't run it. The nightly stress job + manual
    `gh workflow run devnexus-common-stress.yml` runs it against a real
    PG via the GitHub Actions postgres service.

Validation (when first added):
    This test must be verified to **segfault against the pre-fix
    devnexus-common main** (the bug must be reproducible). Then verify it
    passes against the fix. If the test doesn't fail against the unfixed
    code, it's a worthless test.
...
# skip — once a real PG is in scope, every test must exercise it.
...
def _build_manager():
    """Construct a real DatabaseManager from the env vars."""
    # Import here (not at module top) so the import doesn't fail in
    # environments that lack asyncpg installed (e.g. lightweight CI).
...
def db_manager():
    """Module-scoped fixture: build the manager and connect once."""
    mgr = _build_manager()
...
# ---------------------------------------------------------------------------
# The bug-reproducing test
# ---------------------------------------------------------------------------


def test_concurrent_cancellation_does_not_segfault(db_manager):
    ...
    query_delay = float(os.environ.get("STRESS_QUERY_DELAY", "0.5"))  # pg_sleep seconds
    ...
    def _do_query(idx: int) -> Tuple[int, str]:
        ...
        try:
            result = db_manager.fetchval_sync("SELECT pg_sleep($1)::text", query_delay)
    ...
    print(
        f"\n[concurrent_cancellation] N={N} ok={len(ok)} err={len(err)} "
        f"cancelled={cancelled_count} elapsed={elapsed:.2f}s"
    )
    if err:
...
# ---------------------------------------------------------------------------
# Lighter regression: small N for fast CI runs
# ---------------------------------------------------------------------------


def test_minimal_concurrent_burst_does_not_segfault(db_manager):
    """Smaller, faster version for CI.

    Runs 8 concurrent fetchval_sync calls, cancels 4. If the dedicated
    loop + CancelledError handling is broken, this small test still
    segfaults (the original bug was reproducible with N=2, not just N=50).
    """
    N = 8
